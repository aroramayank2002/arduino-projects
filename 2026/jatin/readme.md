...
| # | Module | Sketch | Interface / pins | What the sketch proves |
|---|---|---|---|---|
...
| 2 | BH1750 light / lux | [`BH1750_Light_lux_Sensor`](BH1750_Light_lux_Sensor) | I²C · `0x23` | Continuous lux readings to serial |
| 3 | DHT11 temp + humidity | [`DHT11_Tem._Humidity`](DHT11_Tem._Humidity) | digital 13 | °C and %RH on a 2 s cycle |
| 4 | HTU21D (HW-220) temp + humidity | [`HW220_Humidity_Temp`](HW220_Humidity_Temp) | I²C · `0x40` | °C and %RH on a 2 s cycle — the higher-accuracy alternative to the DHT11 |
| 5 | INA219 current / voltage | [`INA219_current_voltage`](INA219_current_voltage) | I²C · `0x40` | Bus voltage, shunt mV, load voltage, mA and mW — the part that measures what the rest of the kit draws |
| 6 | L298N dual H-bridge | [`L298N_MOTOR_DRIVER`](L298N_MOTOR_DRIVER) | EN 3 (PWM) · IN 4 · IN 7 · pot on A0 | Direction control plus live speed from a potentiometer, mapped to a 70–255 duty floor so the motor still turns at the bottom of the range |
...
| 12 | SG90 servo | [`servo_motor`](servo_motor) | pin 10 | Lock position, 90° step, and a smooth degree-by-degree 0–90 sweep |
| 13 | HC-SR04 + servo gate | [`ultrasonic_servo`](ultrasonic_servo) | TRIG 9 · ECHO 8 · servo 10 | Distance from `pulseIn`, converted at 0.034 cm/µs, driving a gate with **hysteresis** — opens under 10 cm, closes only past 15 cm, so it cannot chatter at the threshold |

### Integration tests

These two are the ones that matter — they prove the parts co-exist, not just that each works alone.

| Module | Sketch | What it proves |
|---|---|---|
| Three I²C devices, one bus | [`Practical_I2c_comunication`](Practical_I2c_comunication) | BH1750 `0x23`, SSD1306 OLED `0x3C` and PCA9685 `0x40` sharing one SDA/SCL pair. Lux is read, mapped to a servo pulse (150–600) and to degrees, the servo moves, and both numbers print to the OLED. One bus, three addresses, no collisions. |
| Mega testing jig | [`Testing_Jig_ArduinoMega`](Testing_Jig_ArduinoMega) | A serial menu that exercises three subsystems on demand: `1` runs four L298N motors (pins 39–45), `2` reads the MAX6675 (46/47/48), `3` sweeps all 16 PCA9685 channels through their full 150–600 travel. |
