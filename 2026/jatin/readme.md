...
|---|---|---|---|---|---|
| 1 | ST7735 1.8″ TFT, 128×160 | [`128_160_TFT_MODULE`](128_160_TFT_MODULE) | SPI |  | Full-screen colour fills, border, text at two sizes, `FreeSans9pt7b` / `FreeMono9pt7b` fonts, portrait + landscape rotation, and it renders the 10-item kit test menu |
...
| 3 | DHT11 temp + humidity | [`DHT11_Tem._Humidity`](DHT11_Tem._Humidity) | 1-wire |  | °C and %RH on a 2 s cycle |
...
| 6 | L298N dual H-bridge | [`L298N_MOTOR_DRIVER`](L298N_MOTOR_DRIVER) | digital + PWM |  | Direction control plus live speed from a potentiometer, mapped to a 70–255 duty floor so the motor still turns at the bottom of the range |
| 7 | MQ-2 gas | [`MQ_GAS_SENSOR`](MQ_GAS_SENSOR) | analogue |  | Raw analogue gas readings |
| 8 | MAX6675 + K-type probe | [`Max6675_thermocouple`](Max6675_thermocouple) | soft SPI |  | °C and °F, with the 250 ms settling the part needs between reads |
| 9 | Rain / water sensor | [`Rain_Water_Sensor`](Rain_Water_Sensor) | analogue |  | Threshold detection — above 100 counts raises the alarm output |
| 10 | TTP223 capacitive touch | [`TOUCH_TTP223`](TOUCH_TTP223) | analogue |  | Latching toggle: one touch on, next touch off, debounced by a state flag rather than a blocking wait |
| 11 | Reed switch module | [`read_switch_module`](read_switch_module) | digital |  | Active-low magnetic read driving an LED |
| 12 | SG90 servo | [`servo_motor`](servo_motor) | PWM |  | Lock position, 90° step, and a smooth degree-by-degree 0–90 sweep |
| 13 | HC-SR04 + servo gate | [`ultrasonic_servo`](ultrasonic_servo) | digital + PWM |  | Distance from `pulseIn`, converted at 0.034 cm/µs, driving a gate with **hysteresis** — opens under 10 cm, closes only past 15 cm, so it cannot chatter at the threshold |
...
| Mega testing jig | [`Testing_Jig_ArduinoMega`](Testing_Jig_ArduinoMega) | SDA 20 · SCL 21 | A serial menu that exercises three subsystems on demand: `1` runs four L298N motors (pins 39–45), `2` reads the MAX6675 (46/47/48), `3` sweeps all 16 PCA9685 channels through their full 150–600 travel. |
